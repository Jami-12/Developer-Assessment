...
const metrics = async () => {
  const [
    ...
    prisma.companyProfile.count({ where: { isDeleted: false } }),
    prisma.user.count({ where: { role: "CANDIDATE", isDeleted: false } }),
...
export const AdminService = { metrics, createAudit, listAudits };
